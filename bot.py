...
async def add_pair_point(
    ...
    try:
        ...
        await channel.send(
            ...
            f"👤 {user1.mention} <:azumeo_y:1538431979165589505> <:azumeo_e:1538431708481847379> <:azumeo_u:1538431970260946956> {user2.mention}\n"
            f"🎭 Role: `{role_text}`\n"
            f"➕ **+{amount} điểm**\n"
            f"🏆 Điểm chung: **{total}**\n"
            f"📌 {reason}"
...
async def on_ready():
    ...
    print(
        "🏷️ Tag: +1 điểm / 5 phút / cặp"
    )

    print(
        "🏆 100 giờ Voice → tự cấp role"
    )
